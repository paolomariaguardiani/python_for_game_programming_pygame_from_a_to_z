...
def display_game_over():
    ...
    pygame.display.flip() # .flip() to see the text on the screen (update the screen)
...
while run: 
    ...
    pygame.time.delay(30) # we delay the snake speed of 30 milliseconds

    # Check for collision with the snake's body
    for block in snake_list[:-1]:
        if block == snake_head: # for me is better: if snake_head == block:
            display_game_over()
            pygame.time.delay(1000)
            run = false
